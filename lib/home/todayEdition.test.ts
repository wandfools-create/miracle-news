...
describe("todayEdition", () => {
  ...
  it("0 today articles with prior edition → carryover featured from previous edition", () => {
    const articles = [
      card({
        id: "yesterday-top",
        is_top_story: true,
    ...
    assert.equal(edition.editionTitleKo, "최근 주요 기사");
    assert.equal(edition.editionTitleEn, "Recent top story");
    assert.match(edition.preparingPhaseKo, /새로운 오늘 기사 준비 중/);
  });
