...
import { endOfPriorNyEditionMs } from "./nyEditionTime";
...
/**
 * When today has no stories, reproduce the prior edition featured + secondary
 * using the same selection policy at the prior edition boundary (not current now).
 */
export function pickPreviousEditionFeatured(
  ...
  /** Last instant of the prior NY edition — avoids re-scoring with current clock. */
  const referenceNowMs = endOfPriorNyEditionMs(editionDateKey);

  const editionArticles = articlesForEditionDateKey(articles, priorKey);
  const core = filterHomeCoreEligible(editionArticles, referenceNowMs);
  const featured = pickFeaturedArticle(core, referenceNowMs);
  if (!featured) return null;

  let secondaryFeatured: HomeArticleCard | null = null;
  let featuredRelated: HomeArticleCard[] = [];
  if (editionArticles.length >= 2) {
    const hub = pickFeaturedHubArticles(core, featured, {
