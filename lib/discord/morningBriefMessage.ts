...
export type MorningBriefMessageState = "active" | "shortlisted" | "dismissed";
...
export function buildMorningBriefPayload(
  ...
  return {
    content: formatMorningBriefMessageContent(item, state),
    components: buildMorningBriefComponents(item.id, item.originalUrl, state),
  };
}
