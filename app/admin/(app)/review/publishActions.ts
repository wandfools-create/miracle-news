...
/** 검토 완료 및 공개 — atomic approve+publish from pending review. */
...
export async function mobileRejectFromForm(formData: FormData) {
  ...
  if (!articleId) redirect("/admin/review/mobile?error=missing");
  if (!reason) {
    redirect(mobileReviewUrl(articleId, { error: "반려 사유를 입력해 주세요." }));
  }
...
export async function mobileRequestRevisionFromForm(formData: FormData) {
  ...
  if (!articleId) redirect("/admin/review/mobile?error=missing");
  if (!note) {
    redirect(mobileReviewUrl(articleId, { error: "수정 요청 내용을 입력해 주세요." }));
  }
...
/** Desktop review detail — same publish action, returns to detail page. */
export async function reviewCompleteAndPublishDetailFromForm(formData: FormData) {
