...
const actionBtn =
  "flex min-h-[48px] w-full items-center justify-center rounded-xl px-4 py-3 text-base font-semibold transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2";
...
export default function MobileReviewDetail({
  ...
  return (
    <main className="min-h-screen bg-neutral-50 pb-32 text-neutral-950">
      ...
      <footer className="fixed inset-x-0 bottom-0 z-30 border-t border-neutral-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-lg flex-col gap-2 px-4 py-3">
          {showReject ? (
            <form action={mobileRejectFromForm} className="space-y-2">
              ...
              <input type="hidden" name="nextArticleId" value={neighbors.nextId ?? ""} />
              <textarea
                name="rejectedReason"
                value={rejectReason}
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className={`${actionBtn} bg-red-700 text-white`}
                >
                  반려 확정
                </button>
              </div>
            ...
            <>
              <form action={reviewCompleteAndPublishFromForm}>
                ...
                <button
                  type="submit"
                  disabled={!contentOk || (sameEventBlocked && !overrideSameEvent)}
                  className={`${actionBtn} bg-black text-white disabled:cursor-not-allowed disabled:opacity-40`}
                >
                  검토 완료 및 공개
                </button>
              </form>
